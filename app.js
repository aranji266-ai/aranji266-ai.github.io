...
const isHistory=w=>w.format==='SCANNED_DOCUMENT'||w.type==='历史档案';
...
function renderWork(w,i){const history=isHistory(w),cover=(w.content||[])[0]||w.image;return `<article class="work-card archive-file${history?' historical-archive':''}" tabindex="0" data-index="${i}"><div class="file-tab">${safe(w.id)} / ${safe(history?w.status||'DECRYPTED':'SEALED')}</div><div class="work-art" ${cover?`style="background-image:url('${safe(assetUrl(cover))}');background-size:cover;background-position:top center"`:''}><span class="archive-stamp">${history?'DECRYPTED<br>DATA RECOVERED':'ARCHIVED<br>ACCESS LOGGED'}</span></div><div class="file-index"><span>${safe(w.id)}</span><span>${history?`${(w.content||[]).length} SCANS`:'CLASSIFIED'}</span></div><h3>${safe(w.title)}</h3><span class="work-type">${safe(w.type)} / ${safe(w.author)}</span></article>`}
function openViewer(w){
  if(isHistory(w)){const pages=(w.content||[w.image]).filter(Boolean);$('#viewerBody').innerHTML=`<div class="viewer-inner history-viewer"><header class="history-meta"><span class="eyebrow">${safe(w.id)} / ${safe(w.status||'DECRYPTED')} / ${(pages.length)} SCANS</span><h2>${safe(w.title)}</h2><span class="work-type">${safe(w.type)} / ${safe(w.author)}</span></header><div class="scan-pages">${pages.map((src,i)=>`<figure><span>SCAN ${String(i+1).padStart(2,'0')} / ${String(pages.length).padStart(2,'0')}</span><img src="${safe(assetUrl(src))}" alt="${safe(w.title)} 第 ${i+1} 页" loading="${i?'lazy':'eager'}"></figure>`).join('')}</div></div>`
  }else{const src=assetUrl(w.image);const media=w.mediaType==='video'&&src?`<div class="viewer-art"><video controls playsinline src="${safe(src)}"></video></div>`:`<div class="viewer-art" ${src?`style="background-image:url('${safe(src)}');background-size:contain;background-repeat:no-repeat;background-position:center"`:''}><span>${safe(w.id)} / SEALED FILE</span></div>`;$('#viewerBody').innerHTML=`<div class="viewer-inner">${media}<div class="viewer-copy"><span class="eyebrow">${safe(w.id)} / CLASSIFIED / ${safe(w.type)}</span><h2>${safe(w.title)}</h2><span class="work-type">ARCHIVED BY: ${safe(w.author)}</span></div></div>`}
  $('#viewer').showModal();
}
async function boot(){
  ...
  const works=cloud?.works?cloud.works.map(w=>w.id==='ARCHIVE_HIST_001'?(staticWorks.find(x=>x.id===w.id)||w):w):staticWorks;
  ...
  $('#workTrack').innerHTML=works.map(renderWork).join('');document.querySelectorAll('.work-card').forEach(c=>{const open=()=>openViewer(works[+c.dataset.index]);c.onclick=open;c.onkeydown=e=>{if(e.key==='Enter')open()}});
}
