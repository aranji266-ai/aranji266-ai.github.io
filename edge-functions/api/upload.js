...
export async function onRequestPost({request}){
  ...
  if(mode==="start"){
    ...
    const uploadId=`${Date.now()}-${crypto.randomUUID()}`;return json({uploadId,publicUrl:`/api/media/${encodeURIComponent(`multipart/${uploadId}.manifest.json`)}`});
  }
  if(mode==="complete"){
    const uploadId=String(input.uploadId||"");const parts=Number(input.parts||0),size=Number(input.size||0);if(!/^[0-9-]+$/.test(uploadId)||!parts||parts>30)return json({error:"分片参数不正确"},400);
...
export async function onRequestPut({request}){
  ...
  if(uploadId||part!==null){if(!/^[0-9-]+$/.test(uploadId||"")||!/^[0-9]{1,2}$/.test(part||""))return json({error:"分片参数不正确"},400);const bytes=await request.arrayBuffer();if(!bytes.byteLength||bytes.byteLength>4*1024*1024)return json({error:"分片大小不正确"},413);await store().set(`multipart/${uploadId}/part-${String(part).padStart(3,"0")}`,bytes,{cacheControl:"public, max-age=31536000"});return json({ok:true})}
